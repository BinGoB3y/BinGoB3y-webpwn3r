...
#updates:
# 1- Boş parametre sorunu düzeltildi.
# 2- User agents when sending a Request. 
# 3- Hata Tabanlı SQLI Algılama Desteği Eklendi.
# 4- Çerezler, Refere ve UserAgent'a XSS İnjection eklenmeye çalışıldı.
...
def main_function(url, payloads, check):
	...
	vuln = 0
        if opener.code == 999:
                # Algılama WebKnight WAF StatusCode'dan.
                print ga.red +" [~] WebKnight WAF Algılandı"+ga.end
                print ga.red +" [~] Her istek arasında 3 saniye gecikme"+ga.end
                time.sleep(3)
		...
		html = request.readlines()
                for line in html:
                    ...
                    if len(checker) !=0:
                        ...
                        vuln +=1
        if vuln == 0:                
        	print ga.green+" [!] Hedefde Açık Bulunamadı!"+ga.end
        else:
        	print ga.blue+" [!] Helal Olsun Karşim %i Tane Açık Buldun :-) " % (vuln) +ga.end

# Burada güvenlik açıkları işlevleri ve algılama yükleri bulunur. 
def rce_func(url):
	headers_reader(url)
